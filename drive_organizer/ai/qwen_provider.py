...
class QwenFlashProvider:
    ...
    def classify_batch(
        ...
        if not requests_list:
            return []
        raw = _call(settings.qwen_flash_model, _build_prompt(requests_list, strategy_hint, allowed_folders))
        return _parse(raw, requests_list, "qwen-flash")


class QwenProProvider:
    def health_check(self) -> bool:
        return bool(settings.dashscope_api_key)
    ...
    def classify_batch(
        ...
        if not requests_list:
            return []
        raw = _call(settings.qwen_pro_model, _build_prompt(requests_list, strategy_hint, allowed_folders))
        return _parse(raw, requests_list, "qwen-pro")
    ...
    def parse_custom_taxonomy(self, description: str) -> dict:
        ...
        raw = _call(settings.qwen_pro_model, prompt)
        raw = re.sub(r"```[a-z]*\n?", "", raw).strip()
        try:
